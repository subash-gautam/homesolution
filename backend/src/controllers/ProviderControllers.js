...
export const updateProvider = async (req, res) => {
  ...
  if (
    ...
    return res.status(400).json({ error: "Nothing is subjected to change !!" });
  }
  ...
  try {
    ...
    const updatedProvider = await prisma.provider.update({
      ...
      data: {
        ...
        lat: lat !== undefined && lat !== "" ? parseFloat(lat) : provider.lat,
        lon: lon !== undefined && lon !== "" ? parseFloat(lon) : provider.lon,
...
export const getDocuments = async (req, res) => {
  ...
  const { id, providerId, status, uploadedAt, lastReview, comment } = req.query;
