...
const Pprofile = () => {
  ...
  const fetchProviderData = async () => {
    try {
      ...
      // Add profile field if it exists in the response
      const providerWithProfile = {
        ...parsedData,
        profile: parsedData.profile || null, // Ensure profile key exists
      };

      // Save updated provider data to AsyncStorage
      await AsyncStorage.setItem(
        "providerData",
        JSON.stringify(providerWithProfile)
      );
